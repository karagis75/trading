...
def yahoo_symbol(symbol: str) -> str:
    ...
    return cleaned  # Direct fallback if user enters ticker like 'CL=F' directly
...
def check_bearish_regime(symbol: str) -> bool:
    ...
    try:
        ...
        print(f"Warning: Could not check weekly status due to: {e}. Defaulting to Bullish.")
        return False
...
def latest_impulse(pivots: list[dict], bearish: bool) -> tuple[dict, dict] | None:
    """Find either the newest confirmed L->H or H->L swing based on the target regime."""
...
def calculate_fib_levels(pivots: list[dict], bearish: bool) -> tuple[dict, dict, dict] | None:
    ...
    if bearish:
        # Bearish layout: H -> L. Retracements go UP from the Low. Targets extend DOWN from the High.
        ...
        # Bullish layout: L -> H. Retracements pull DOWN from the High. Targets extend UP from the Low.
...
def plot_chart(frame: pd.DataFrame, symbol: str, pivot_width: int, is_bearish: bool, output: Path, show: bool = False) -> None:
    ...
    if fib:
        ...
        # Color coding adjustments for trend structures
        ret_color = "#f472b6" if is_bearish else "#60a5fa"      # Pinkish (Bear) vs blue (Bull)
        target_color = "#ef4444" if is_bearish else "#22c55e"   # Red (Bear) vs green (Bull)
        invalid_color = "#22c55e" if is_bearish else "#ef4444"  # Green upper ceiling vs red lower floor
...
def main() -> None:
    ...
    # 1. Evaluate market context from weekly profile
    is_bearish = check_bearish_regime(args.symbol)
    
    # 2. Extract operational OHLC dataset maps
