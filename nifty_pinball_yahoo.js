...
// Hardcoded array for Nifty 50 and Bank Nifty Yahoo symbols
...
const DELAY_MS             = parseInt(process.env.DELAY_MS             || '300', 10); // Elevated slightly to bypass index scraping blocks
...
function downloadYahooData(yahooSymbol, period1, period2) {
    // CRITICAL: Caret symbols require raw inclusion or alternate formatting to avoid query parsing rejections
...
async function fetchIndexHistoryFromYahoo(symbol) {
    ...
    // Calculate historical UNIX timestamps
    ...
    try {
        ...
        console.warn(`[YAHOO-ERR] Failed to download structure details for ${symbol}: ${err.message}`);
        return [];
    }
}
...
function analyzeFibPinball(symbol, rows) {
    ...
    for (let i = pivots.length - 1; i >= 2; i--) {
        ...
        if (curPrice < w2c.price) {
            ...
            if (daysSinceW2 <= 30 && curPrice > w2c.price) {
                ...
                waveConfidence  = 55;
                waveDescription = `Possible early w1 of Wave 3; index bounced from W2 (${r2(w2c.price)}) but not yet above W1 high (${r2(w1c.price)})`;
            }
        } else {
            ...
            if (extRatio <= 0.618) {
                ...
                waveConfidence  = 65;
                waveDescription = `In sub-wave 1 of Wave III; index (${r2(curPrice)}) broke above W1 high (${r2(w1c.price)}) at ${r2(extRatio * 100)}% of W1 amplitude from W2 (target range: 0.382-0.618 ext)`;
            } else if (extRatio <= 1.236) {
                waveLabel       = 'Wave 3';
                waveConfidence  = 80;
                waveDescription = `In Wave III (strongest wave); index (${r2(curPrice)}) at ${r2(extRatio * 100)}% Wave III targets: 1.0 ext=${r2(levels.e1_000)} to 1.618 ext=${r2(levels.e1_618)}`;
                ...
                if (w4Pullback) {
                    ...
                    waveConfidence  = 75;
                    waveDescription = `In extended Wave III; index (${r2(curPrice)}) at ${r2(extRatio * 100)}% ext; extended target 1.618 ext=${r2(levels.e1_618)}`;
                }
                ...
                waveConfidence      = 50;
                waveDescription     = `Index beyond 2.0 ext (${r2(extRatio * 100)}% of W1 amplitude from W2); potential blow-off or start of new higher-degree wave`;
            }
        }
    ...
    if (!best) {    
        best = detectEarlyWave1(cleanSymbol, rows, useRows, curPrice, curDate);
    }
    return best;
}

function detectEarlyWave1(cleanSymbol, rows, useRows, curPrice, curDate) {
    ...
    const priorLow   = Math.min(...priorBars.map(r => r.low));
    ...
    return {
        ...
        Confidence:      60,
        Description:     `Early Wave 1: fresh structure low at ${r2(w0Low)} (${daysSinceW0} bars ago); index gained ${r2(gainFromW0 * 100)}% from W0; Wave 1 high so far: ${r2(highAfterW0)}`,
        'W0 Low':        r2(w0Low),
        'W0 Date':       w0Date,
...
async function main() {
    ...
    console.log(`Targeting Indices: ${INDEX_SYMBOLS.join(', ')}`);
    console.log(`Lookback Configuration: ${LOOKBACK_DAYS} days from today.`);
    
    // Clear any corrupted local index cache files to force refresh
    ...
    const outAll    = path.join(OUT_DIR, 'fib_index_results_all.csv');
    const outWave1  = path.join(OUT_DIR, 'fib_index_results_wave1.csv');
    const outWave3  = path.join(OUT_DIR, 'fib_index_results_wave3.csv');
    const outWave5  = path.join(OUT_DIR, 'fib_index_results_wave5.csv');
    ...
    console.log('\n================================================================================');
    console.log(' FIBONACCI PINBALL INDEX RESULTS (YAHOO FINANCE)');
    console.log('================================================================================');
    if (allResults.length === 0) {
        console.log('No structures matched the parameters for standard extensions.');
    } else {
        allResults.forEach(r => {
            console.log(
                `${r.Symbol.padEnd(12)} | Position: ${r['Wave Position'].padEnd(18)} | ` +
