...
import bullishbiasnifty500 as scanner
...
class MainCliEngineTests(unittest.TestCase):
    def test_main_reads_csv_without_excel_format_error(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            out = Path(tmp) / "Bullish_Bias_Analysis.xlsx"
            SAMPLE_TICKERS.to_csv(src, index=False)

            def fake_analyze(symbol: str, config: scanner.BullishScannerConfig):
                return {"Ticker": symbol, "ADX": 30.0, "CCI": 120.0}

            with patch.object(scanner, "analyze_symbol", side_effect=fake_analyze):
                with patch(
                    "sys.argv",
                    ["bullishbiasnifty500.py", "--input", str(src), "--output", str(out)],
                ):
